...
export class ProKitchenTransformerService {

  // -----------------------------------------------------------------
  // DI
  // -----------------------------------------------------------------
  constructor(
    private parseService: ProKitchenParseService,
    ...
    private xmlExtractorService: ProkitchenXmlExtractorService,
  ...
  public async transform(request: TransformationRequest): Promise<void> {
    ...
    const parsedXml = await this.parseService.parse(request);
    console.log("parsedXml: \n%o", parsedXml);

    // 2) Extract to temporary model
    const extractedOrder: ExtractedOrder =  this.xmlExtractorService.extract(parsedXml);
